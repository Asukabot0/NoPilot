...
Dispatch a fresh Codex sub-agent for the tracer bullet. Pass the tracer module list and an available platform.

Use the installed Codex prompt:
```
Prompt file: ~/.codex/prompts/lash-tracer.md
Inputs:
- Tracer modules: <module_ids>
- Platform: <platform>
- Project root: <cwd>
```
...
Dispatch a fresh Codex sub-agent for the batch:
```
Prompt file: ~/.codex/prompts/lash-batch.md
Inputs:
- Batch: <N>
- Modules: <module_ids>
...
If batches are independent, you MAY dispatch multiple batch agents in parallel using Codex sub-agents. However, each batch must complete before the next dependent batch starts (respect the topological ordering from the plan).
...
Dispatch a fresh Codex sub-agent for verification:
```
Prompt file: ~/.codex/prompts/lash-verify.md
Inputs:
- Project root: <cwd>
```
