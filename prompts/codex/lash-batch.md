...
For each module in this batch, follow the EXACT same process as described in `~/.codex/prompts/lash-tracer.md` steps 1-9:
...
- **If merge conflict occurs**: spawn a conflict-resolution Worker in a new worktree (use `~/.codex/prompts/lash-conflict-resolver.md` as instruction) to resolve, then re-test and re-merge.
